...
/*
atomicAdd 是CUDA提供的原子操作函数，用于在全局内存中实现线程安全的加法操作。它的核心特性是：
  原子性：确保多线程并发访问时，加法操作不会被中断
  返回值：返回操作前内存地址的原始值
当执行 atomicAdd(&(y[a[idx]]), 1); 时，GPU会执行以下步骤：
  读取旧值：从 y[a[idx]] 读取当前计数值（记为 old）
  计算新值：执行 new = old + 1
  写入并返回：将 new 写入 y[a[idx]]，并返回 old
*/
...
#define TORCH_BINDING_HIST(packed_type, th_type, element_type, n_elements)     \
  torch::Tensor histogram_##packed_type(torch::Tensor a) {                     \
    ...
    // result 的类型：std::tuple<torch::Tensor, torch::Tensor>
    // std::get<0>(result): 最大值张量
    // std::get<1>(result): 最大值索引张量                                              
