...
GELU 精确计算公式：
$$ GELU(x) = xP(X<=x) = x\frac{1}{\sqrt{2\pi}}\int_{-\infty}^{x}\exp^{-\frac{t^2}{2}}dt = \frac{1}{2} \left(1 + erf (\frac{x}{\sqrt{2}}) \right) $$
...
对于float32数据，在计算tanh(x)的过程中直接调用cuda内置的`tanhf`函数，CUDA 提供的 设备端（device-side）单精度 tanh 函数，专为在 GPU kernel 内调用而设计。

对于半精度数据，没有内置的函数计算 tanh，需要自己实现。
